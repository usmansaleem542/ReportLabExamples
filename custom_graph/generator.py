...
def GenerateData(filename='sample_data2.json', offset = 50, randomN = False):
    ...
    for i in range(len(x)):
        ...
        if randomN: q1 = [np.random.randint(val - offset, val), np.random.randint(val, val + offset)]
        else: q1 = [val - offset, val + offset]
        data["Q1"].append(q1)
        if randomN: q2 = [np.random.randint(q1[0] - offset, q1[0]), np.random.randint(q1[1], q1[1] + offset)]
        else: q2 = [q1[0] - offset, q1[1] + offset]
        data["Q2"].append(q2)
